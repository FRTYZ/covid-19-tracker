...
export function* getCountriesSaga(): Generator<any, void, any> {
  try {
      // Request yardımcı fonksiyonunu kullanarak API isteğini gerçekleştirdik
      const data = yield call(Request, {
        method: 'GET',
        url: '/countries',
      });

      yield put(getCountriesSuccess(data)); // Veri başarılı şekilde alındığında bir action dispatch edildi
  } catch (error) {
      yield put(getCountriesFailure(error)); // Hata oluştuğunda bir action dispatch edildi
  }
}
